...
### 🛠️ Limitações Metodológicas e Próximos Passos

Como o banco de dados SIM/TCE-CE agrega os valores de Ordens de Serviço de forma unificada no exercício de 2025, o TCO apresentado reflete o comportamento macroeconômico do Estado. 

Registra-se como limitação do atual estágio do projeto a ausência de uma quebra volumétrica automatizada por tipo de veículo (leves vs. pesados) no cálculo do TCO. Contudo, a arquitetura modular desenvolvida em Python (`src/etl.py`) já foi desenhada para receber, em uma próxima iteração, o atributo `tp_veiculo` como nova dimensão de agrupamento, permitindo isolar a viabilidade financeira por categorias específicas e refinar ainda mais o suporte à decisão gerencial.
